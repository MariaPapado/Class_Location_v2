...
class Trainer(object):
    ...
    def validation(self):
        ...
        ids = os.listdir(self.args.dataset_path + '/images/')
        for itera, id in enumerate(tqdm(ids)):
            
            pre_change_imgs = Image.open(self.args.dataset_path + '/images/{}'.format(id))
            labels_loc = Image.open(self.args.dataset_path + '/masks/{}'.format(id))

            pre_change_imgs, labels_loc = np.array(pre_change_imgs), np.array(labels_loc)   
            pre_change_imgs = self.normalize_img(pre_change_imgs)
            ...
            cv2.imwrite('./PREDS/{}'.format(id), output_loc*255)
        ...
        return loc_f1_score, loc_recall_score, loc_prec_score
...
def main():
    ...
    parser.add_argument('--dataset_path', type=str, default='/notebooks/xBD/val')
...
#    parser.add_argument('--test_data_list_path', type=str, default='/data/ggeoinfo/datasets/xBD/xBD_list/val_all.txt')
    ...
    parser.add_argument('--resume', type=str, default='/notebooks/MambaCD/changedetection/saved_models/xBD/MambaBDA_Small_1715699448.5963047/25500_model.pth')

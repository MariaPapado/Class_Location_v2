...
def random_fliplr_bda(pre_img, label_1):
    if random.random() > 0.5:
    ...
    return pre_img, label_1

def augment_img(in_img, label, idx):
#        print('shape', in_img)
        in_img = np.array(in_img, dtype=np.uint8)
        in_img = Image.fromarray(in_img)
        if idx==0: 
            #print('0')
            in_img = F.adjust_brightness(in_img, random.uniform(0.7, 0.9))  #0.5-1.5, 1.5 too much maybe
                                                       #0.8-1.4 
        elif idx==1:
            #print('1')
            in_img = F.adjust_contrast(in_img, random.uniform(0.5, 0.9))  #0.5-1.5, 1.5 too much maybe
...
#        elif idx==2:
#            #print('2')
#            in_img = F.adjust_saturation(in_img, random.uniform(0.7, 1.4))

#        elif:
#            #print('3')
#            inputs = F.adjust_hue(in_img, random.uniform(-0.5, 0.5))

        in_img = np.asarray(in_img, dtype=np.float32)
        in_img = in_img.clip(0,255)
...
def random_flipud_bda(pre_img, label_1):
    if random.random() > 0.5:
...
def random_rot_bda(pre_img, label_1):
    k = random.randrange(3) + 1
...
def random_crop_bda(pre_img, loc_label, crop_size, mean_rgb=[0, 0, 0], ignore_index=255):
    h, w = loc_label.shape
